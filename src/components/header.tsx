...
function Header() {
  const [ratingVisible, setRatingVisible] = useState(false);
  const [rating, setRating] = useState(0); // Lägger till ett nytt state för att spara betyget

  const toggleRatingVisibility = () => setRatingVisible(!ratingVisible);

  // Funktion för att hantera klick på en stjärna och uppdatera betyget
  const handleRating = (index) => {
    setRating(index + 1); // Uppdaterar betyget till det valda indexet + 1
  };
