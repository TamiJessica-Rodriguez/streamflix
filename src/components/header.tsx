...
const FEATURE_FLAG_RATING_SYSTEM = true; // Antag att funktionen är aktiverad
...
function Header() {
  ...
  useEffect(() => {
    // Endast visa stjärnbetyg på PreviewPage
    if (location.pathname === "/PreviewPage") {
      setRatingVisible((prev) => prev); // Behåll nuvarande tillstånd när sidan laddas
    } else {
      setRatingVisible(false); // Dölj betygsstjärnorna på andra sidor
    }
  }, [location]);
